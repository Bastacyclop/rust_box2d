...
use math::Vec2;
use dynamics::{BodyMutRef, BodyConstRef};
...
pub trait MutJoint: WrappedMutBase<ffi::Joint> {
    fn body_a(&mut self) -> BodyMutRef {
        unsafe {
            WrappedMut::from_ptr(
    ...
    fn mut_next(&mut self) -> UnknownJointMutRef {
        unsafe {
...
pub trait ConstJoint: WrappedJoint+WrappedBase<ffi::Joint> {
    ...
    /*
    unsafe fn next(&self) -> UnknownJointRef {
        unsafe {
            WrappedConstBase::from_ptr(
...
//impl<'l> ConstJoint for UnknownConstJointRef<'l> {}
