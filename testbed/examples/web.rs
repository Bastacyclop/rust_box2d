...
fn create_joints(world: &mut World,
    ...
    for (i, &(ax, ay, bx, by)) in [(-10., 0., -0.5, -0.5),
                                   (10., 0., 0.5, -0.5),
                                   (10., 20., 0.5, 0.5),
                                   (-10., 20., -0.5, 0.5)].iter().enumerate() {
        create_joint(ground, bodies[i],
                     b2::Vec2 { x: ax, y: ay }, b2::Vec2 { x: bx, y: by });
    }

    for (i, &(ax, ay, bx, by)) in [(0.5, 0., -0.5, -0.),
                                   (0., 0.5, 0., -0.5),
                                   (-0.5, 0., 0.5, 0.),
                                   (0., -0.5, 0., 0.5)].iter().enumerate() {
        create_joint(bodies[i], bodies[(i + 1) % bodies.len()],
                     b2::Vec2 { x: ax, y: ay }, b2::Vec2 { x: bx, y: by });
    }
}
